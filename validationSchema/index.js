...
export const signUpSchema = Yup.object().shape({
    fname: Yup.string()
    .matches(/^[A-Za-z][A-Za-z\s]*$/, 'First name can only have alphabets')
      .required('First name is required'),
    lname: Yup.string()
    .matches(/^[A-Za-z][A-Za-z\s]*$/, 'Field can only have alphabets')
      .required('Field is required'),
    phone: Yup.string()
    .matches(/^\d{11,14}$/, 'Phone number must be between 11 and 14 digits')
